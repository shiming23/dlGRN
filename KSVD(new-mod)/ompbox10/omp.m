...
% A small modification that each signal is equal likely represented by 2-T
% atoms(T>2)

% DtX,G

global randpermAtom
global randpermAtom_IX
% randpermAtom = randperm(size(DtX,2));
...
if T > 1
    ...
    for tempT = 2: T
        ...
        temp_min = 1+round(size(DtX,2)/(T-1)*(tempT-2));

        temp_max = round(size(DtX,2)/(T-1)*(tempT-1));

        gamma = [gamma, ompmex(D,X,DtX(:,randpermAtom(temp_min:1:temp_max)),G,tempT,sparse_gamma,msgdelta,profile)];

    end
    
    gamma(:,randpermAtom) = gamma;
